...
from src.dlcv.config import get_cfg_defaults
#from src.dlcv.dataset import SubsetSTL10
from src.dlcv.model import DeepLabV3Model
from src.dlcv.utils import *
from src.dlcv.training import train_and_evaluate_model
...
def main(cfg):#, stratification_rates):
    ...
    train_dataset = VOCSegmentation(root=cfg.DATA.ROOT, year='2012', image_set='train', download=True, transform=train_transform, target_transform=train_transform)
    test_dataset = VOCSegmentation(root=cfg.DATA.ROOT, year='2012', image_set='val', download=True, transform=test_transform, target_transform=test_transform)
    # ToDo
    ...
    train_losses, test_losses, test_accuracies = train_and_evaluate_model(model, train_loader, test_loader,
                                                                          criterion, optimizer, cfg.TRAIN.NUM_EPOCHS,
...
if __name__ == '__main__':
    ...
    config_file_path = os.getenv('CONFIG_FILE', 'configs/config_1.yaml')
